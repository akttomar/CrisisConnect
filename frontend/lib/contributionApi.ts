...
export async function volunteerForIncident(incidentId: string) {
  const token = getAuthToken();
  // Use the incidents route for volunteering
...
export async function unvolunteerFromIncident(incidentId: string) {
  const token = getAuthToken();
  // Use the incidents route for unvolunteering
  const response = await fetch(`${API_BASE_URL}/api/incidents/${incidentId}/volunteer`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` },
  });
