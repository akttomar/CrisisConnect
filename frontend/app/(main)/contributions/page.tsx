...
import { WelcomeCardSkeleton, MapSkeleton } from "@/components/ui/skeleton"
...
export default function ContributionsPage() {
  ...
  const [volunteeredIncidents, setVolunteeredIncidents] = useState<any[]>([])
  const [stats, setStats] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  const [deletingIncidentId, setDeletingIncidentId] = useState<string | null>(null)
  const [cancellingIncidentId, setCancellingIncidentId] = useState<string | null>(null)
  ...
  const handleCancelVolunteer = async (incidentId: string) => {
    ...
    try {
      ...
      toast({
        title: "Cancel failed",
        description: error.message || "Unable to cancel your commitment.",
        variant: "destructive",
      })
    } finally {
                  ...
                  {(stat.title === "Resolved Reports" || stat.title === "Volunteer Hours") && (
                    ...
                    >
                      <div className="w-full bg-muted/30 rounded-full h-2 overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${stat.title === "Resolved Reports" ? (resolvedReports / totalReports) * 100 : (totalVolunteerHours / 20) * 100}%` }}
        ...
        >
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
            ...
            <TabsContent value="overview" className="space-y-8">
              ...
              >
                {/* Enhanced Monthly Contributions Chart */}
                <motion.div
                ...
                {/* Enhanced Contribution Types Pie Chart */}
                <motion.div
                  whileHover={{ scale: 1.02 }}
                  transition={{ duration: 0.3 }}
                >
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie
                              ...
                              labelLine={false}
                            >
                              {contributionTypes.map((entry: any, index: number) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={entry.color}
                                  stroke="#fff"
              ...
              >
                <Card className="bg-gradient-to-br from-card via-card to-muted/20 border-border hover-lift relative overflow-hidden">
                  ...
                  <CardHeader className="relative z-10">
                    ...
                    <CardDescription className="text-muted-foreground">
                      Your progress towards community service goals and achievements
                    </CardDescription>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-6 relative z-10">
                    ...
                    >
                      ...
                      <div className="relative">
                        <Progress
                          value={(resolvedReports / totalReports) * 100}
                          className="h-3"
                        />
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${(resolvedReports / totalReports) * 100}%` }}
                    ...
                    >
                      ...
                      <div className="relative">
                        <Progress
                          value={(totalVolunteerHours / 20) * 100}
                          className="h-3"
                        />
                        <motion.div
                          initial={{ width: 0 }}
                    ...
                    >
                      {[
                        ...
                        >
                          <motion.div
                            animate={{ rotate: badge.earned ? [0, 5, -5, 0] : 0 }}
                            transition={{ duration: 2, repeat: badge.earned ? Infinity : 0, ease: "easeInOut" }}
                            className={`p-2 rounded-full ${badge.earned ? 'bg-accent/20' : 'bg-muted/50'}`}
                          >
            ...
            {/* Enhanced Reported Problems Tab */}
                    ...
                    {reportedIncidents.map((incident, index) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3 flex-wrap">
                            <motion.div
                              ...
                              className={`px-3 py-1.5 rounded-full text-sm font-medium ${getPriorityColor(incident.priority)} shadow-sm`}
                            >
                              {incident.type}
                            </motion.div>
                            <motion.div
                              whileHover={{ scale: 1.05 }}
                              className={`px-3 py-1.5 rounded-full text-sm font-medium ${getStatusColor(incident.status)} border`}
                            >
                              <div className="flex items-center gap-2">
                                <motion.div
                                  animate={{ rotate: [0, 5, -5, 0] }}
                          ...
                          <motion.div
                            ...
                            className="text-xs text-muted-foreground flex items-center gap-1 bg-muted/50 px-2 py-1 rounded-full"
                          >
                        ...
                        <motion.h3
                          ...
                          className="font-semibold text-foreground mb-3 leading-relaxed group-hover:text-primary transition-colors"
                        >
                        ...
                        {/* Delete button for non-resolved incidents */}
                        ...
                        {/* Impact indicator for resolved incidents */}
                        {incident.status === 'Resolved' && (
                          <motion.div
                            initial={{ scaleX: 0 }}
                            animate={{ scaleX: 1 }}
              ...
              >
                <Card className="bg-gradient-to-br from-card via-card to-muted/20 border-border hover-lift relative overflow-hidden">
                  ...
                  <CardHeader className="relative z-10">
                    ...
                    <CardDescription className="text-muted-foreground">
                      Incidents where you volunteered your time and skills to help the community
                    </CardDescription>
                  </CardHeader>
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3 flex-wrap">
                            <motion.div
                              whileHover={{ scale: 1.05 }}
                              className="px-3 py-1.5 rounded-full text-sm font-medium bg-secondary/20 text-secondary border border-secondary/30 shadow-sm"
                            >
                              {incident.type}
                            </motion.div>
                            <motion.div
                        ...
                        <motion.h3
                          ...
                          animate={{ opacity: 1 }}
                          transition={{ delay: 0.1 + index * 0.05 }}
                          className="font-semibold text-foreground mb-4 leading-relaxed group-hover:text-secondary transition-colors"
                        >
                          {incident.description}
                        </motion.h3>
